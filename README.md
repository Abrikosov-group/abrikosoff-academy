...
подтверждение через Telegram. Локальная приёмка включается только явно:

```bash
ADMINISTRATION_ENABLED=true npm run dev
```
...
production `/admin` этого этапа остаётся закрыт до приёмки второго канала
входа и независимых security-уведомлений.
