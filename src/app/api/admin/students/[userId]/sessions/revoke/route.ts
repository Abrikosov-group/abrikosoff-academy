...
type RevokeSessionsRouteContext = {
  params: Promise<{ userId: string }>;
};

type RevokeSessionsBody = {
  reason?: unknown;
};
...
function validateBody(value: unknown): RevokeSessionsBody {
  if (
    !value ||
    typeof value !== "object" ||
    Array.isArray(value)
  ...
  return value as RevokeSessionsBody;
}

export async function POST(
  request: Request,
  routeContext: RevokeSessionsRouteContext,
) {
  ...
  try {
    ...
    const { service, revokeUserSessionsService } =
    ...
    const result = await revokeUserSessionsService.execute({
      context,
      targetUserId: userId,
      reason: body.reason,
    ...
    if (error instanceof AdministrationError) {
      if (securityErrorCodes.has(error.code)) {
        logSecurityEvent(
          "administration.revoke_sessions_rejected",
          {
            code: error.code,
            requestId,
            userAgentFamily,
        ...
        error.code === "REVOKE_USER_SESSIONS_FAILED"
      ) {
        logUnexpectedServerError(
          "administration.revoke_sessions_failed",
          error,
        );
      }
