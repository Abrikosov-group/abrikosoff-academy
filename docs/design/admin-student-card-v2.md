...
- Статус: реализовано локально, функциональная QA-проверка пройдена
- Дата: 2026-07-30
- Функциональная QA-проверка: владелец, 2026-07-30,
...
3. внутри группы — от новой к старой по `createdAt`.

Пока фоновое обновление `lastSeenAt` не реализовано, это поле не используется
для сортировки и не показывается как фактическая активность пользователя.
...
| Время | `Сессия создана 30 июл., 02:17 · истекает 29 авг.` |
...
- полные даты создания, истечения и отзыва;
- полный IP;
...
### 13.3. Ручная функциональная QA-проверка
...
- три контура ревью текущего SHA по
  [порядку разработки и выпуска](../development-workflow.md) не содержат
  открытых блокирующих замечаний;
- функциональная QA-проверка фактически пройдена владельцем на постоянном
  локальном HTTPS-адресе.
...
7. Зафиксировать проверяемую версию и провести параллельный раунд CI, трёх
   ревью и пользовательской проверки по
   [порядку разработки и выпуска](../development-workflow.md).
8. Устранить замечания, ответить в обсуждениях и повторять раунды для каждого
   нового значимого SHA до выполнения критериев готовности к слиянию.
