...
Для явной локальной приёмки достоверного дашборда, списка и карточки ученика
владелец на время сессии может установить
`ADMINISTRATION_MODE=operational`. Это исключение действует только для точного
origin `academy-dev.abrikosoff.com`, пока в принимаемом пакете нет изменяющих
административных маршрутов. После проверки гейт возвращается в `false`, а
режим — в `owner_preview`. Production-origin остаётся в `owner_preview` до
завершения обязательных зависимостей этапа 2 административного ТЗ.
