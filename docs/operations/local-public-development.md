...
Production-подобная сборка и туннель запускаются только на время QA-сессии и
останавливаются сразу после неё. Постоянным остаётся DNS-адрес, а не
...
Для отдельной проверки `/admin` на время QA-сессии устанавливается
`ADMINISTRATION_ENABLED=true`. Режим `owner_preview` открывает на локальном
origin для QA только защитный фундамент Administration.

Для явной локальной функциональной QA-проверки операционных возможностей,
включая дашборд, карточки и изменяющие команды, разработчик / Codex либо
владелец на время сессии может установить
...
## 3. Локальная разработка и функциональная QA-проверка
...
После завершения QA-сессии останавливаются оба процесса:
